...
export default {
  ...
  async cancelFlight(req, res) {
    ...
    try {
      const result = await bookingFlight.cancelFlightService(
        userId,
        transactionId
      );
      res.status(200).json({
      ...
      console.error(
        "Error in FlightCancellationController.cancelFlightOrder:",
        error
      );
      if (
        error.message === "Transaction not found" ||
        error.message === "User not found for transaction"
      ) {
        return res.status(404).json({ message: error.message, data: null });
      }
      ...
      if (
        error.message.includes("Cancellation window has passed") ||
        ...
        error.message.includes(
          "Airline partner not found for refund debiting"
        ) ||
  ...
  async cancelRoom(req, res) {
    ...
    try {
      const result = await bookingServices.cancelRoomService(
        userId,
        transactionId
      );
      res.status(200).json({
      ...
      console.error(
        "Error in HotelCancellationController.cancelHotelReservation:",
        error
      );
      if (
        error.message === "Transaction not found" ||
